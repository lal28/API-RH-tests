  ...
  describe('3. TESTE DE CAIXA-PRETA - UPDATE', () => {
    ...
    describe('Comportamento: Entrada Inválida + Usuário Existente', () => {
      ...
      test('deve rejeitar atualização com idade inválida', async () => {
        ...
        expect(response.body).toHaveProperty('error', 'Dados inválidos');
        expect(response.body.details).toContain('Idade deve ser um número entre 0 e 120');
      });
  ...
  describe('4. TESTE END-TO-END - FLUXO COMPLETO CRUD', () => {
    ...
    describe('Jornada Completa do Usuário', () => {
      ...
      test('deve executar fluxo completo: CREATE → READ → UPDATE → DELETE', async () => {
        ...
        const usuarioNaLista = finalListResponse.body.data.find(u => u.id === usuarioId);
    ...
    describe('Múltiplos Usuários - Integridade do Sistema', () => {
      ...
      test('deve manter integridade ao manipular múltiplos usuários', async () => {
        ...
        await request(app)
          .get(`/api/v1/users/${usuarios[0].id}`)
          .expect(200);
          
        await request(app)
          .get(`/api/v1/users/${usuarios[2].id}`)
          .expect(200);
          
        // Verificar que o deletado não existe mais
        await request(app)
          .get(`/api/v1/users/${usuarioMeio.id}`)
          .expect(404);
      });
  ...
  describe('5. TESTE DE VERIFICAÇÃO VS VALIDAÇÃO - DELETE', () => {
    ...
    describe('VERIFICAÇÃO: Estamos deletando o usuário CERTO?', () => {
      ...
      test('deve deletar APENAS o usuário específico solicitado', async () => {
        ...
        const usuariosQueDevemPermanecer = usuariosParaTeste.filter(u => u.id !== usuarioParaDeletar.id);
      ...
      test('deve manter integridade dos dados de outros usuários', async () => {
        ...
        const verificacao1 = await request(app).get(`/api/v1/users/${usuarioOriginal1.id}`).expect(200);
        const verificacao2 = await request(app).get(`/api/v1/users/${usuarioOriginal2.id}`).expect(200);
        const verificacao3 = await request(app).get(`/api/v1/users/${usuarioOriginal3.id}`).expect(200);
      ...
      test('deve manter contagem correta na listagem após deleção', async () => {
        ...
        const usuarioNaLista = listaDepois.body.data.find(u => u.id === usuarioParaDeletar.id);
        expect(usuarioNaLista).toBeUndefined();
      });
    ...
    describe('VERIFICAÇÃO + VALIDAÇÃO: Cenários Complexos', () => {
      ...
      test('deve manter sistema estável após múltiplas deleções', async () => {
        ...
        await request(app).delete(`/api/v1/users/${usuarios[1].id}`).expect(200); // Delete 2º
        await request(app).delete(`/api/v1/users/${usuarios[3].id}`).expect(200); // Delete 4º
        ...
        await request(app).delete(`/api/v1/users/${usuarios[1].id}`).expect(404);
        await request(app).delete(`/api/v1/users/${usuarios[3].id}`).expect(404);
      });
